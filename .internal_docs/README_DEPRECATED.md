...
- Phase 09: answer-level evaluation and artifact-driven visualization

Phase 07 is now complete in intended scope:

- Phase 07A added the committed golden eval set, build manifest, and saved baseline eval artifact
- Phase 07B extracted explicit internal interfaces for embedders, rerankers, query analysis, retrievers, and answer backends
- Phase 07C wired named alternate provider selections without changing defaults
- Phase 07 cleanup closed the remaining CLI, artifact-state, and interface-story congruence gaps

Phase 07 cleanup is now implemented:

- the eval CLI reranker surface now matches the retrieval and answer CLIs
- answer backend typing now uses the shared interface story consistently
- the repo docs now distinguish the Snowflake code/config default from the currently committed local `hashed_v1` dense artifact snapshot
- implementation handoff is captured in `PHASE_07_CLEANUP_HANDOFF.md`

Phase 07A is now implemented:

- committed golden eval set at `eval/golden_queries.json`
- machine-readable build manifest emitted to `artifacts/build_manifest.json`
- bounded eval runner available as `uv run eliza-rag-eval`
- saved baseline retrieval run at `eval/baseline_targeted_hybrid_retrieval.json`

Phase 07B is now implemented:

- explicit internal interfaces now exist for embedders, rerankers, query analysis, retrievers, and answer backends
- current defaults were moved behind those seams without changing CLI behavior
- implementation handoff is captured in `PHASE_07B_INTERFACE_EXTRACTION_HANDOFF.md`

Phase 07C is now implemented:

- explicit named embedder selections:
  - `snowflake-arctic-embed-xs`
  - `bge-m3`
- explicit named reranker selections:
  - `bge-reranker-v2-m3`
  - `bge-reranker-base`
- dense-index build support for side-by-side alternate embedding artifacts
- documented baseline versus candidate comparison commands

Phase 09 is now implemented in bounded form:

- saved answer-included baseline artifact at `eval/provider_baseline_snowflake_bge_v2_m3_answer.json`
- saved answer-included comparison artifacts at:
...
- structured answer-quality judging now records:
  - groundedness
  - citation quality
  - usefulness
  - comparison completeness
  - uncertainty handling
- quantitative OpenRouter judge overlays now record:
  - per-dimension `0-5` scores
  - aggregated weighted overall answer scores
  - derived `pass`, `partial_pass`, and `fail` statuses from the atomic scores
- answer judging links to `eval/answer_judging_rubric.md`
- read-only artifact reporting is available as `uv run eliza-rag-eval-report`
- generated report artifact saved at `eval/provider_eval_report.md`
- judged-only report artifact saved at `eval/provider_eval_report_judged.md`
- OpenRouter-backed LLM judge architecture is now available as `uv run eliza-rag-eval-judge`
- default judge target:
  - provider: `openrouter`
  - model: `z-ai/glm-5`

What is intentionally still in progress after the bounded Phase 09 pass:

- deciding whether to refresh the committed local dense artifact snapshot from `hashed_v1` to a published Snowflake-based release artifact
- a later query-routing or metadata-inference comparison slice if the team wants a third visualization axis beside embedder and reranker comparisons
...
- the code and config default embedder alias is `snowflake-arctic-embed-xs`
- the currently committed local dense artifact contract in `artifacts/dense_index_metadata.json` still describes a `hashed_v1` index
- the bounded Phase 08 embedder comparison that actually landed is `snowflake-arctic-embed-xs` versus `hashed_v1`
- `bge-m3` remains wired as an optional candidate, but it was not kept in the saved Phase 08 evidence set because the local rebuild cost was too high for the bounded slice
- the built-in `hashed_v1` embedding path remains available as a deterministic fallback and as the current saved local artifact baseline
...
The current retrieval and answer path is good enough to stabilize before attempting broader modularization.

Recommended next sequence:

1. preserve the committed golden eval set and saved baseline as the regression contract
2. keep the extracted provider interfaces stable as the experiment surface
3. use the saved Phase 08 provider artifacts as the initial evidence layer:
...
4. use the saved answer-included artifact set plus quantitative judged overlays as the answer-quality evidence layer:
...
5. build any visualization or reporting layer directly from saved `eval/*.json` artifacts
6. only then decide whether any default recommendation or committed release artifact baseline should change

Comparison axes for the saved evidence:
...
- answer-included artifacts currently saved:
  - `snowflake-arctic-embed-xs` + `bge-reranker-v2-m3`
  - `hashed_v1` + `bge-reranker-v2-m3`
  - `hashed_v1` + `bge-reranker-base`
  - `snowflake-arctic-embed-xs` + `bge-reranker-base`
- the fourth condition is a combined-condition matrix expansion rather than a one-variable-at-a-time comparison axis
- no saved `bge-m3` embedder eval artifact exists in the evidence set

Why this order:

- the repo now has a credible demo path, so the main risk is regression during refactor rather than missing baseline functionality
- the repo can now save and inspect a stable baseline artifact before modularization work
- model swaps are easier to judge now that the repo has both a measurable baseline and explicit provider seams
- the Phase 08 scoring layer now records explicit `pass`, `partial_pass`, and `fail` outcomes plus contamination observations, so saved retrieval-only provider runs are already more defensible than the old placeholder contract
- the Phase 09 answer artifact shows that retrieval wins do not automatically become answer-quality wins
- deterministic query analysis should remain the default until the evaluation data shows that routing is the real bottleneck
- the current committed local dense artifact baseline should not be rewritten just because Snowflake is the code/config default
...
Reranking is now available as an explicit retrieval option. The default reranker is `BAAI/bge-reranker-v2-m3`, while the older deterministic `heuristic` reranker remains available as a fallback. The first alternate reranker candidate is `bge-reranker-base`, exposed behind the same selection surface.
...
export ELIZA_RAG_JUDGE_MODEL=qwen/qwen3.6-plus:free
uv run eliza-rag-eval-judge \
  eval/provider_baseline_snowflake_bge_v2_m3_answer.json \
  --output eval/provider_baseline_snowflake_bge_v2_m3_answer_judged.json
```

## Provider Experiment Prep

Phase 07C leaves the default path unchanged while wiring one alternate embedder and one alternate reranker for one-at-a-time comparisons.

Named provider selections:
...
- run `uv sync` first so `sentence-transformers`, `transformers`, and `torch` are available
- the first run of `--embedder bge-m3`, `--reranker bge-reranker-v2-m3`, or `--reranker bge-reranker-base` will download model artifacts from Hugging Face
- if you want side-by-side dense experiments, give the alternate embedder its own dense table and metadata artifact name

Exact comparison commands:

Baseline dense build:
...
- the alternate providers are wired in and selectable
- this section documents comparison commands and prerequisites
- this change does not claim that `bge-m3` or `bge-reranker-base` has already won an evaluation
- the documented default dense path is Snowflake, but the committed local dense artifact snapshot is still `hashed_v1` until a Snowflake rebuild is saved
...
- do the next engineering pass as stabilization plus modularization prep, not as a broad learned-routing rewrite
...
- The repo now emits `artifacts/build_manifest.json` to record chunking, dense-index, reranker, and artifact identity together.
- The repo now includes a committed golden eval set at `eval/golden_queries.json` and a saved baseline output at `eval/baseline_targeted_hybrid_retrieval.json`.
- `uv run eliza-rag-eval` emits a structured payload with config, retrieved tickers, retrieved chunk ids, optional answer fields, and saved scoring outcomes.
- `uv run eliza-rag-build-dense-index` refreshes the build manifest as part of the normal dense-artifact build flow.
